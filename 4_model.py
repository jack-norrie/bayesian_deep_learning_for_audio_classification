...
def gen_wind_mel_cnn_insp(input_shape=(128, 128, 2), num_classes=50,
    ...
    model = Sequential([
        Input(shape=input_shape, dtype='float32'),
        Conv2D(filters=8, kernel_size=(16, 16), strides=(1, 1),
               activation='relu',
               kernel_regularizer=regularizers.l2(reg)),
        Conv2D(filters=8, kernel_size=(16, 16), strides=(1, 1),
               activation='relu',
               kernel_regularizer=regularizers.l2(reg)),
        MaxPool2D(pool_size=(8, 8), strides=(4, 4)),
        BatchNormalization(),
        Dropout(rate=dor),
        Conv2D(filters=16, kernel_size=(4, 4), strides=(1, 1),
               activation='relu',
               kernel_regularizer=regularizers.l2(reg)),
        Conv2D(filters=16, kernel_size=(4, 4), strides=(1, 1),
               activation='relu',
        ...
        BatchNormalization(),
        Dropout(rate=dor),
        Conv2D(filters=32, kernel_size=(2, 2), strides=(1, 1),
               activation='relu',
               kernel_regularizer=regularizers.l2(reg)),
