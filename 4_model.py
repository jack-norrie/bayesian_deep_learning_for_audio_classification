...
def gen_wind_mel_cnn_insp(input_shape=(128, 128, 2), num_classes=50,
    ...
    model = Sequential([
        Input(shape=input_shape, dtype='float32'),
        BatchNormalization(),
        Conv2D(filters=32, kernel_size=16, strides=1,
               activation='elu',
               kernel_regularizer=regularizers.l2(reg)),
        Conv2D(filters=32, kernel_size=16, strides=1,
               activation='elu',
               kernel_regularizer=regularizers.l2(reg)),
        MaxPool2D(pool_size=8, strides=4),
        BatchNormalization(),
        Conv2D(filters=64, kernel_size=4, strides=1,
               activation='elu',
               kernel_regularizer=regularizers.l2(reg)),
        Conv2D(filters=64, kernel_size=4, strides=1,
               activation='elu',
        ...
        BatchNormalization(),
        Conv2D(filters=128, kernel_size=2, strides=(1, 1),
               activation='elu',
               kernel_regularizer=regularizers.l2(reg)),
        AvgPool2D(pool_size=7, strides=7),
        Flatten(),
        BatchNormalization(),
        Dropout(rate=dor),
